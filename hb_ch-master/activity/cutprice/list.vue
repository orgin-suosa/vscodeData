	...
	<view>
		<view class="m-product-all">
						...
						<navigator :url="'/activity/cutprice/detail?mid=' + item.activity_id + '&pid=' + item.item_id" class="m-product-item m-product-GP">
							...
							<view class="m-product-info">
								<view class="m-product-name">
									<label><text class='u-tuan-label'>{{__('砍')}}</text>{{item.activity_rule.item_name}}</label>
									<view class="groupNumber">
										<day-countdown :timer="(item.activity_endtime)" :Type="1"></day-countdown>
									</view>
								</view>
								<view class="m-product-price" style='font-size:38upx'><label style='color:#888888;margin-right:10upx;'>{{__('底价')}}</label><label>{{__('￥')}}</label>{{item.activity_rule.cut_down_min_limit_price}}
			...
			<view :class="(tapindex==2?'':'hide')" style="font-size:28upx">
				<scroll-view scroll-y="true" v-if="(fglist.length>0)" class="m-orderlist" @scrolltolower="scrollbottomtwo" style="width:100%;height:100%;position:absolute;padding-top:66upx;box-sizing: border-box;">
						...
						<view class="m-product-item m-product-GP">
							...
							<view class="m-product-info">
								...
								<view class="m-product-price">
                                    <label style='color:#888888;margin-right:10upx;'>{{__('底价')}}</label><label>{{__('￥')}}</label>{{item.activity_rule.cut_down_min_limit_price}}
	...
	export default {
		...
		methods: {
            ...
            toBeGroupLists: function () {
                this.setData({tapindex: 2, page: 1, rows: 10, fglist: [], activity_state: this.StateCode.ACTIVITY_STATE_WAITING}), this.InitPaging()
            },
            //saas,调用当前参与的团
            InitPaging: function () {
...
<style lang="scss">
	...
	.m-product-price .u-btn{padding:0;margin: 0 20upx;width: 160upx;float: right;font-size: 24upx;line-height: 50upx;margin-top: 10upx;overflow: hidden;height: 50rpx;}
