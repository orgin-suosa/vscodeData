<template>
	<view>
		<!-- #ifdef APP-PLUS -->
		<view style="background-color: rgb(186,61,67);height: 40rpx;"></view>
		<!-- #endif -->
		<view class="course_nav">
			<view class="course_nav_left" @click="goback" style="width: 65rpx;height: 65rpx;">
				<text class="iconfont icon-fanhui" style="font-size: 32rpx"></text>
			</view>
		</view>
		<view class="lecturer_info" :style="{'height':userInfoHeight+'px'}">
			<image class="user_avatar" :src="lecturerDetail.lecturer_blank_avatar">
				<view class="nick_name">{{lecturerDetail.user_nickname}}</view>
				<u-button class="sub_btn" type="primary" shape="circle" @click="subLecturer(lecturerDetail.user_id)">
					{{subText}}</u-button>
				<!-- <view class="lecturer_introduct" v-html="lecturerDetail.lecturer_introduce"> -->
				<view class="lecturer_introduct">
					<text class="yd-content" v-html="tempContent"></text>
					<div class="yd-content-left" v-if="txt_content !== null && txt_content.length > 45">
						<text class="yd-quanwen" style="float: right; margin-right: 20rpx;"
							v-if="isShowAllContent">简介详情</text>
						<text class="yd-quanwen" style="float: right; margin-right: 20rpx;" v-else
							@click="toggleDescription(txt_content.length)">收起</text>
						<image class="yd-content-img" v-if="!isShowAllContent" lazy-load src="/static/images/next.png"
							@click="toggleDescription(txt_content.length)"></image>
						<image class="" v-else lazy-load src="/static/images/next.png"></image>
					</div>
				</view>
		</view>
		<view class="context_box">
			<view class="course-tabs" style="margin: 0 30rpx;height:20rpx">
			</view>
			<view>
				<view class="flex-col" v-if="current === 0">
					<view class="u-btns1">
						<u-button type="default" :class="btns1Index === 0 ? 'u-btns1-1-active' : 'u-btns1-1'"
							text="精彩回放" @click="changeBtnStyle1(0)">
							<image src="https://qnsp.zcskjy.com/zc_images/edu_pic/video.png" class="icon_image">
								<text style="margin-left: 30rpx;">精彩回放</text></image>
						</u-button>
						<u-button type="default" :class="btns1Index === 1 ? 'u-btns1-1-active' : 'u-btns1-1'" text="节目单"
							@click="changeBtnStyle1(1)">
							<image src="https://qnsp.zcskjy.com/zc_images/edu_pic/videolist.png"
								class="icon_image"><text style="margin-left: 30rpx;">节目单</text></image>
						</u-button>
					</view>
						...
						<view class="playback-area-list" v-if="btns1Index === 0 && wonderfulPlayback.length >0">
							<view class="playback-area-content" @click="videoJump(item.pageUrl, item.state)"
								...
								<view class="playback-area-child">
									...
									<image class="list-live-image" :src="item.image" mode="widthFix"></image>
								</view>
							...
							<view class="" v-for="(item, index) in playbackAreaList" :key="index">
								...
								<view class="playback-area-content" @click="videoJump(item.pageUrl, item.state)">
									<view class="playback-area-child">
										...
										<image class="list-live-image" :src="item.image" mode="widthFix">
										</image>
									</view>
									<view class="playback-area-describe">
										<view class="playback-area-describe-text">{{ item.describe }}</view>
					...
					<!-- <view v-show="liveBackVideo === 0">
						<view class="u-btns2">
							<u-button type="default" :class="btns2Index === 0 ? 'u-btns2-2-active' : 'u-btns2-2'" size="small" shape="circle" text="默认" @click="changeBtnStyle2(0)"></u-button>
							<u-button type="default" :class="btns2Index === 1 ? 'u-btns2-2-active' : 'u-btns2-2'" size="small" shape="circle" text="最新" @click="changeBtnStyle2(1)"></u-button>
							<u-button type="default" :class="btns2Index === 2 ? 'u-btns2-2-active' : 'u-btns2-2'" size="small" shape="circle" text="最热" @click="changeBtnStyle2(2)"></u-button>
						</view>
						<view class="live_video_items" v-for="item in liveVideoList" v-show="1 == 2">
							<view class="flex-col-content">
								<image class="col_image" :src="item.imageUrl"></image>
								<view class="col_text1">{{item.name}}</view>
								<view class="col_text2">{{item.introduction}}</view>
								<view class="col_text3">主讲：{{item.lecturer}}</view>
							</view>
						</view>
					</view>
					<view v-show="liveBackVideo === 1">				
						<picker class="weui-btn" @change="PickerChange" :value="indexPicker" :range="array">
							  <view class="time_spicker" @click="show = true">今天 ▼</view>
						</picker>
						<view class="live_video_items" v-for="item in liveVideoList">
							<view class="videoTime">{{item.time}}</view>
							<view class="flex-col-content" v-show="1 == 2">
								<image class="col_image" :src="item.imageUrl"></image>
								<view class="col_text1">{{item.name}}</view>
								<view class="col_text2">{{item.introduction}}</view>
								<view class="col_text3">主讲：{{item.lecturer}}</view>
				...
				<view class="flex-col" v-else>
					<z-paging ref="paging"
						...
						empty-view-img="https://qnsp.zcskjy.com/zc_images/edu_pic/edu_19.png"
						:refresher-enabled="false" :use-page-scroll="true" v-model="courseList"
						:empty-view-fixed="false" @query="getCourseList" :fixed="false"
						...
						<view class="course_list" v-for="item in courseList">
							<view class="flex-col-content" @click="jumpDetail(item)">
								...
								<view class="col_text2" style="height:38rpx;line-height: 38rpx;overflow: hidden;">
									{{item.product_tips}}</view>
								...
								<u-button class="col_text5" type="default" shape="circle" size="small">
									<u-icon :label="item.courseItems" labelSize="16rpx" labelColor="#5B2111" size="12"
										name="https://qnsp.zcskjy.com/zc_images/edu_pic/video_tag.png"></u-icon>
									<view v-if="item.chapter">
										<text style="margin-left: 10rpx;">{{item.chapter.chapterNum}}节课时</text>
									</view>
								</u-button>
								...
								<view class="col_text4">
									...
									<view v-else>
										<text v-if="item.product_unit_price != 0" style="font-size: 40rpx;"><span
...
<script>
	...
	export default {
		...
		onLoad(options = {}) {
			...
			that.getLecturerDeatil(options)
			that.isSubLecturer(options)
			that.getHighlightsList(options)
		},
		methods: {
			getPrice(price) {
				if (price) {
				...
				that.$.request({
					...
					success: function(data, status, msg, code) {
						if (status == 200) {
							...
							if (txtCntIndex > 43) {
								that.isShowAllContent = true
								that.tempContent = that.txt_content.substr(0, 43) + "..."
							} else {
								that.isShowAllContent = false
								that.tempContent = that.txt_content
							}
