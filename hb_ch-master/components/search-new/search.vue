<template name="zy-search">
  <view>
      ...
      <image v-if="searchText.length > 0" src="https://qnsp.zcskjy.com/zc_images/images/false.png" mode="aspectFit" @click="clearSearch()" class="clear-icon"></image>
    </view>
    <view :class="'s-' + theme" v-if="hList.length > 0 && showSearch && isHistory">
      <view class="header">
        <view>{{ __('搜索历史') }}</view>
        <!-- <image src="https://static.shopsuite.cn/xcxfile/appicon/zy-search/delete.svg" mode="aspectFit" @click="delhistory('center')"></image> -->
        <image src="https://qnsp.zcskjy.com/zc_images/home/icon-clear.png" mode="aspectFit" @click="delhistory('center')"></image>
      </view>
      <view class="list hList">
        <view v-for="(item, index) in hList" :key="index" @click="keywordsClick(item)">
