  ...
  <view class="content banner-wrap">
    ...
    >
      <swiper-item v-for="(item, index) in bannerList" :key="index">
        <view class="banner-swiper-item" :class="swiperConfigure.current == index ? '' : 'active'">
          <maskTitle :state="item.state" :number="item.number"> </maskTitle>
          <image :src="item.image" mode="widthFix" class="banner-swiper-img"></image>
          <view class="item-right" @click.stop="videoJump(item.page_url)">
            ...
            <view class="live-button" v-if="item.state === 1">
              <image class="icon-live2" src="${getApp().globalData.qnUrl}/zc_images/findPage/live1.png" mode="widthFix"></image>
              <text>直播</text>
            </view>
            <view class="playback-button" v-if="item.state === 2">
              <image class="icon-live2" src="${getApp().globalData.qnUrl}/zc_images/findPage/live3.png" mode="widthFix"></image>
