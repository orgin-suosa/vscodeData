<template>
	<view class="page">
		<view v-if="(isdata)" class="m-ad-list">
			<view v-for="(item, i) in addresslist"  :key="i" class="m-ad-item" :data-id="(item.ss_id)"  :data-ud_id="(item.ss_id)" @longpress="modify">
				<view class="m-ad-l"  :data-sel="ud_id" :data-ud_id="(item.ss_id)"   @tap="SelectAddress">
					<view class="m-ad-i-top">
						<text>{{item.ss_name}}</text><label>{{item.ss_mobile}}</label>
					</view>
					<view class="m-ad-i-bot">
						<label v-if="(item.ss_is_default)" class="m-ad-icon">{{__('默认')}}</label><text>{{item.ss_province}}/{{item.ss_city}}/{{item.ss_county}}  {{item.ss_address}}</text>
					</view>
				</view>
				<view class="m-ad-edit tbl">
					<label :data-ud_id="(item.ss_id)" @click="EditAddress" class="iconfont icon-edit"></label>
					<label class="iconfont icon-delete" :data-ud_id="(item.ss_id)" @click="DelAddress"></label>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			GetAddressList: function() {
				...
				that.$.request({
					url: that.Config.URL.seller.lists_shipping_address,
					data: params,
			...
			EditAddress: function(obj) {
				...
				//console.log(obj.currentTarget.dataset.ud_id)
				options.ud_id = obj.currentTarget.dataset.ud_id;

				if (that.ud_id != 0) {
					that.$.redirectTo({
						url: that.$.createUrl("/seller/store/manage", options)
					});
				} else {
					that.$.navigateTo({
						url: that.$.createUrl("/seller/store/manage", options)
			...
			DelAddress: function(e) {
				...
				that.$.showModal({
					...
					success: function(n) {
						if (n.confirm) {
							var params = {
								ss_id: e.currentTarget.dataset.ud_id
							};
							that.$.request({
