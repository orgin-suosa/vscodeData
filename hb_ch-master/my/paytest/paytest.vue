<template>
	<view class="content">
		<button type="primary" @click="debug()">跳转支付页</button>
		<button type="primary" @click="confirmPayment('wxpay')">微信支付</button>
		<button type="primary" @click="confirmPayment('alipay')">支付宝支付</button>
	</view>
</template>
		...
		methods: {
			...
			async h5OldPay(e) {
				...
				that.$.showModal({
					...
					success: res => {
						if (res.confirm) {
							let redirect_url = that.cf.SiteUrl + '/h5/edu/order/detail?on=' + that
								.order_id;

							if (that.kind == 'order') {} else if (that.kind == 'favorable') {
								redirect_url = that.cf.SiteUrl +
									'/h5/chain/chain/favorabledetail?order_id=' + that.order_id;
							} else {
			...
			async h5Pay(e) {
				...
				if (orderInfo.paid) {
					...
					if (e == 'alipay') {
						...
						let redirect_url = that.cf.SiteUrl + '/h5/edu/order/detail?init_pay_flag=1&on=' + that
							.order_id;
						if (that.kind == 'order') {} else if (that.kind == 'favorable') {
							redirect_url = that.cf.SiteUrl +
								'/h5/chain/chain/favorabledetail?init_pay_flag=1&order_id=' + that.order_id;
						} else {

						}
						//页面已经跳转走了
						window.location.href = orderInfo.mweb_url + '&redirect_url=' + encodeURIComponent(
							redirect_url);
					}
				}
