<template>
	...
	<view class="uni-banner" style="background:#FFFFFF;" v-if="loaded">
		<view :style="'height: ' + h + 'px; width:750uxp;padding:30rpx;'">
		...
		<view style="height:50px;text-align: center;">
			<view class="button-sp-area">
				...
				<button class="u-btn-hlight" @tap="agree">{{__('同意')}}</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			if (options.id == "open_store_description") {
				uni.setNavigationBarTitle({
					title: this.__('用户服务协议')
				});
			} else {
				uni.setNavigationBarTitle({
					title: this.__('用户隐私政策')
				});
			}
		...
		methods: {
			getProtocal: function() {
				let that = this;
				let params = {
					protocols_key: this.protocols_key
				};

				console.log("that.cf.URL.protocol", that.cf.URL.protocol)
