		...
		<view class="header-view" style="background-color: #ffffff">
			...
			<view class="source-view">
				<view class="left-view">
					<!-- 文章开头，缩略图 -->
					<!-- <image class="banner-img" :src="img" mode="widthFix" ></image> -->
					<!-- 文章摘要 -->
					<view class="banner-title">{{ source }}</view>
				</view>
				<view class="right-view">{{ time }}</view>
			</view>
		</view>
		...
		<uni-popup ref="share" type="share" safeArea backgroundColor="#fff">
			<!-- <view class="share-content">
						<view class="share-content-channel">
							...
							<view class="">
								<image src="https://qnsp.zcskjy.com/zc_images/findPage/WechatMoments.png" mode="widthFix"></image>
								<view class="">朋友圈</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		onReady() {
			...
			if (this.id) {
				// ID 不为空，则发起查询
				// console.log('onReady-id--', this.id);
				// this.requestData();
			} else {
				uni.showToast({
					icon: 'none',
		...
		methods: {
			...
			requestData() {
				...
				uni.request({
					method: 'GET',
					data: params,
					url: this.Config.URL.newsUrl.newsdetail,
