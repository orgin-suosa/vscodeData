				...
				<view class='hidden_box_Input' v-if="ishidden">
					<view class='hidden_box_Input_box'>
						<view class='hidden_box_input'>
							<textarea class='input_box_input_input' cursor-spacing="110" disabled="true" fixed="true"
								maxlength="300" :placeholder="__('禁言')" placeholder-style="color:#666;font-size:30rpx"
								:show-confirm-bar="barbar" style='background-colo:#fff;color:#f6f6f6;font-size:30rpx'
								v-if="datauser.IsComment" />

							<textarea @input="textareavalue" class='input_box_input_input' cursor-spacing="45"
								fixed="true" :focus="focus" maxlength="150" :placeholder="huifuvalue"
								placeholder-style="color:#939ca9;font-size:30rpx" show-confirm-bar="bar"
								style='background-colo:#fff' v-model="value2" v-else />
						</view>
					</view>
					<view class='hidden_box_Input_but'><span class='hidden_box_Input_button'
							v-if="datauser.IsComment==true|| value==''">{{__('发送')}}</span>
						<button @click="certificate" class='hidden_box_Input_button' hover-class='none' lang="zh_CN"
							open-type="getUserInfo" style="background-color:#DB384C;line-height:40rpx"
							v-else>{{__('发送')}}
						</button>
					</view>
				</view>
					...
					<view class='box_title'>
						<view @click='gerenzhongxin' style='display:flex;float:left'>
							...
							<view class='box_title_name'>
								...
								<view class='box_title_name'><span
										style="display:block;">{{datauser.story_time_str}}</span>
								</view>
							</view>
						</view>
					</view>
					<view class='box'>
							...
							<view class='newsimg_box_img'>
								<block v-for="(item, idx) in datauser.story_file" :key="idx">
									<view @click='yulan' class='newsimg_box_image3' :data-url='item'
										:style='"background: url("+item+") center no-repeat ;background-size:contain;"'>
									</view>
								</block>
							</view>
						</view>
							...
							<view class='box_title_image2_box'>
								<view v-if="isshowdzphoto">
									<view class='box_title_image2_box_box'>
										...
										<!-- 	<view class='like_user_total'>{{dataContent.like_user_total}}{{__('个赞')}}</view> -->
											<view class='like_user_total'>{{datauser.story_like_count}}{{__('个赞')}}</view> 
								...
								<button @click="onUnLike" class='dzys' :data-story_id='datauser.story_id' lang="zh_CN"
									open-type="getUserInfo" style="background: #fff;padding-left: 0"
								...
								<button @click="onLike" class='dzys' :data-story_id='datauser.story_id'
									hover-class='none' lang="zh_CN" open-type="getUserInfo"
									style='background:#fff;padding-left: 0' v-else>
									<image class='dzys_img'
						...
						<view class='commodity' v-if="dataContent.ProList.length>0">
							<view class='commodity_box'>
								...
								<span class='commodity_text'>{{__('推荐商品')}}</span>
							</view>
								...
								<view class='commodity_shop' v-for="(item, idx) in dataContent.ProList" :key="idx">
									<navigator hover-class="none" :url='"/pages/product/detail?pid=" + item.item_id'>
										<view class='commodity_shopImg'>
											<view class='commodity_shopImg'
												:style='"background: url(" + item.product_image + ") center no-repeat ;background-size:contain;"'>
											</view>
										</view>
										<view class='commodity_shop_name'><span
												class='commodity_shop_nametext'>{{item.product_item_name}}</span>
										</view>
										<view class='commodity_shop_pic'>
											<span>{{sprintf(__('￥%s'), item.product_unit_price)}}</span></view>
									</navigator>
								</view>
							</view>
						</view>
									...
									<view class='rmtz_box_content_box'>
										<view class='rmtz_box_content_box2'>
											...
											<view class='pldz_box'>
												<view class='rmtz_box_content_box_dz'>
													<image class='dz'
														src='https://static.shopsuite.cn/xcxfile/appicon/img/tzxq_03.png'>
													</image>
													<span>{{item.story_brower_count}}</span>
												</view>
												<view class='rmtz_box_content_box_pl'>
													<image class='pl'
														src='https://static.shopsuite.cn/xcxfile/appicon/img/tzxq_05.png'>
													</image>
													<span style='font-size:12px'>{{item.story_comment_count}}</span>
												</view>
												<button @click="onUnLike" class='rmtz_box_content_box_pl'
													:data-story_id='item.story_id' lang="zh_CN" open-type="getUserInfo"
													style='padding-left:22rpx;margin-left:0'
													v-if="item.IsFabulous == 1">
													<image class='pl'
														src='https://static.shopsuite.cn/xcxfile/appicon/img/side1_03.png'>
													</image>
													<span style='font-size:12px'>{{item.story_like_count}}</span>
												</button>
												<button @click="onLike" class='rmtz_box_content_box_pl'
													:data-story_id='item.story_id' lang="zh_CN" open-type="getUserInfo"
													style='padding-left:22rpx;margin-left:0' v-else>
													<image class='pl'
														src='https://static.shopsuite.cn/xcxfile/appicon/img/side1_05.png'>
													</image>
													<span style='font-size:12px'>{{item.story_like_count}}</span>
												</button>
											</view>
										...
										<view @click='remen' class='rmtz_box_content_img' :data-id="item.story_id">
											<view class='rmtz_box_content_Img'
												:style='"background: url(" + item.ImgPath + ") center no-repeat ;background-size:contain;"'>
											</view>
										</view>
									</view>
								</block>
						...
						<view class='rmtz_box' v-if="!datauser.IsComment">
							<view class='commentaries_box'>
								<view class='commentaries2'>
									<view class='commentaries'><span
											class="commentaries_tzpl">{{__('帖子评论')}}</span><span class="all_pl"
									...
									<view class='h_box' v-for="(item, index) in dataComment" :key="index">
										...
										<view class='h_box_neirong'>
											...
											<view class='neirong_time'>
												<view class='neirong_time_time'><span>{{item.comment_time_str}}</span>
												</view>
												<view class='tupianbox'>
													<view @click="onCommentLike" class='tupianbox_img'
														:data-comment_id='item.comment_id' lang="zh_CN"
														open-type="getUserInfo" style="background: #fff;"
														v-if="item.IsFabulous == 0">
														<image class='commentaries_text_time_img'
															src='https://static.shopsuite.cn/xcxfile/appicon/img/side1_05.png'>
														</image>
														{{item.comment_like_count}}
													</view>
													<view @click='onUnCommentLike' class='tupianbox_img'
														:data-comment_id='item.comment_id' v-else>
														<image class='commentaries_text_time_img'
															src='https://static.shopsuite.cn/xcxfile/appicon/img/side1_03.png'>
														</image>
														{{item.comment_like_count}}
													</view>
													<view class="button_reply">
														<image class="reply" @click.stop='ahuifu'
															:data-comment_id='item.comment_id'
															:data-comment_reply_id="item.comment_reply_id"
															:data-user_id='item.user_id'
															:data-user_nickname="item.user_nickname"
															src="https://qnsp.zcskjy.com/zc_images/findTools/pinglun.png">
														</image>
														回复
													</view>
												</view>
											</view>
											<view :class="'zihuifu_box ' + (item.comment_id==comment_id?'':'circle')"
												...
												<block v-for="(items, indexs) in item.commentList" :key="indexs">
													<view class='z_title_box'>
														<view class='z_box_touxiang'>
															<image class='z_box_touxiang' :src='items.user_avatar'>
															</image>
														</view>
													...
													<view class='neirong_time'>
														...
														<view class='tupianbox'>
															...
															<view @click="onReply" class="time_a"
																:data-id="items.comment_reply_id" :data-type="2">
																<image class='ddd'
																	src='https://static.shopsuite.cn/xcxfile/appicon/img/ddd_03.png'>
																</image>
																<view class="triangle_border_right01"
																	v-if="datauser.IsComment&&items.IsShow"></view>
																<view
																	:class="!datauser.IsComment?'hidden_time_a':'falhidden_time_a'"
																	v-if="items.IsShow">
												...
												<view @click='commenTarieHidden' class='shouqi'><span
													...
													<image class='top'
														src='https://static.shopsuite.cn/xcxfile/appicon/img/top.png'>
													</image>
												</view>
											</view>
											<view @click='commentari' class='zihuifu_box' :data-id='item.comment_id'
												v-if="item.commentList.length>0&&item.comment_id!=comment_id">
												<span
													class="zihuifu_dengr">{{sprintf(__('%s等人'), item.reply_name_str)}}</span>
												<span
													class="gonghuifu">{{sprintf(__('共%s条回复'), item.commentList.length)}}</span>
											</view>
										</view>
									</view>
...
<script>
	...
	export default {
		...
		computed: mapState(['Config', 'StateCode', 'notice', 'plantformInfo', 'shopInfo', 'userInfo', 'hasLogin','refreUserspace']),
		components: {
			diy,
			wxParse
		},
		...
		onUnload: function() {
			// 离开页面，注销事件
			var that = this;
			that.$store.commit('updateRefreUserspace',true);
			console.log("离开页面，注销事件",that.refreUserspace)
		...
		methods: {
			...
			certificate: function (id) {
			  ...
			  that.$.request({
			  	...
			  	success: function(data, status, msg, code) {
			  		console.log('---实名验证ss----',data, status, msg, code);
			  		if (status == 200) {
			...
			commentari: function(a) {
				this.setData({
					comment_id: a.currentTarget.dataset.id,
					commentariehidden: !0
				})
			},
			...
			GetMemberPostsInfo: function() {
				...
				that.$.request({
					...
					success: function(data, status, msg, code) {
						if (200 == status) {
							data['story_time_str'] = dateUtil.dateUtils.format(that.$.datetimeFormatter(
								data['story_time']));
							...
							that.userInfo.user_id == that.datauser.user_id ? that.setData({
								isbenren: !0
							}) : that.setData({
								isbenren: !1
							});
			...
			GetPostsCommentByPostsId: function() {
				...
				that.$.request({
					...
					loading: false,
					success: function(data, status, msg, code) {
						console.log("ss:",JSON.stringify(data.items));
						if(JSON.stringify(data.items) =="{}" || JSON.stringify(data.items) == undefined || JSON.stringify(data.items) == null ) return;
						if (data.items.length > 0) {
							...
							for (var r = 0; r < data.items.length; r++) {
								...
								for (var j = 0; j < data.items[r]['commentList'].length; j++) {
									data.items[r]['commentList'][j]['comment_reply_time_str'] = dateUtil
										.dateUtils.format(that.$.datetimeFormatter(data.items[r][
											'commentList'
										][j]['comment_reply_time']));

									//data.items[r]['commentList'][j]['comment_reply_time_str'] = data.items[r]['commentList'][j]['comment_reply_time'];

									data.items[r]['reply_name_str'] = data.items[r]['commentList'][j][
										'user_nickname'
									];
								}
							}
						...
						if (200 == status) {
							for (var t = [], e = 0; e < that.dataComment.length; e++) {
								if (that.dataComment[e].comment_id == comment_id) {
									...
									0 == that.dataComment[e].IsFabulous ? (o.IsFabulous = 1, o
										.comment_like_count = o.comment_like_count + 1) : (o
										.IsFabulous = 0, o.comment_like_count = o.comment_like_count -
										1), t.push(o)
								} else {
					...
					that.$.request({
						...
						success: function(data, status, msg, code) {
							if (200 == status) {
								...
								for (var t = d.currentTarget.dataset.index, e = that.dataComment[t]
										.commentList, o = that.dataComment, n = [], s = 0; s < e
									.length; s++) {
									if (e[s].comment_reply_id == d.currentTarget.dataset
										.comment_reply_id) {
										var r = e[s];
										0 == e[s].IsFabulous ? (r.IsFabulous = 1, r
											.comment_reply_helpful_num = r
											.comment_reply_helpful_num + 1) : (r.IsFabulous =
											0, r.comment_reply_helpful_num = r
											.comment_reply_helpful_num - 1), n.push(r)
									} else {
										n.push(e[s]);
									}
								}
								o[t].commentList = n, that.setData({
									dataComment: o
								})
							} else {}
						}
				...
				that.forceUserInfo(function(user) {
					...
					that.$.request({
						...
						success: function(data, status, msg, code) {
							if (200 == status) {
								for (var t = [], e = 0; e < that.dataComment.length; e++) {
									if (that.dataComment[e].comment_id == n.currentTarget.dataset
										.comment_id) {
										var o = that.dataComment[e];
										0 == that.dataComment[e].IsFabulous ? (o.IsFabulous = 1, o
											.comment_like_count = o.comment_like_count + 1) : (
											o.IsFabulous = 0, o.comment_like_count = o
											.comment_like_count - 1), t.push(o)
									} else {
										t.push(that.dataComment[e]);
									}
								}
			...
			onReply: function(a) {
				...
				for (var i = 0; i < that.dataComment.length; i++) {
					for (var n = 0; n < that.dataComment[i].commentList.length; n++) {
						a.currentTarget.dataset.id == that.dataComment[i].commentList[n].comment_reply_id ? 1 == that
							.dataComment[i].commentList[n].IsShow ? dc[i].commentList[n].IsShow = !1 : dc[i]
							.commentList[n].IsShow = !0 : dc[i].commentList[n].IsShow = !1;
					}

					a.currentTarget.dataset.id == that.dataComment[i].comment_id ? 1 == that.dataComment[i].IsShow ?
						dc[i].IsShow = !1 : dc[i].IsShow = !0 : dc[i].IsShow = !1
				}

				that.setData({
					dataComment: dc
				});
				1 == that.hiddenddd ? that.setData({
					hiddenddd: !1
				}) : that.setData({
					hiddenddd: !0
				})
			...
			ahuifu: function(a) {
				...
				for (var e = that.dataComment, o = 0; o < that.dataComment.length; o++) {
					for (var n = 0; n < that.dataComment[o].commentList.length; n++) {
						a.currentTarget.dataset.id == that.dataComment[o].commentList[n].Id && that.dataComment[o]
							.commentList[n].IsShow, e[o].commentList[n].IsShow = !1;
					}

					a.currentTarget.dataset.id == that.dataComment[o].Id && that.dataComment[o].IsShow, e[o].IsShow = !
						1
				}
				that.setData({
					dataComment: e
			...
			scrollbottom: function(a) {
				if (this.flag && this.ispage) {
					...
					var e = setTimeout(function() {
							t.setData({
								pageIndex: parseInt(t.pageIndex) + 1
							}), t.GetPostsCommentByPostsId()
						},
						500)
				}
			},
			gerenzhongxin: function() {
				let that = this;
				that.$.navigateTo({
					url: "/community/community/userspace?uid=" + this.datauser.user_id
				})
			},
			yulan: function(a) {
				let that = this;

				let url = "/community/community/video?id=" + this.story_id + '&uid=' + that.options.uid;
			...
			remen: function(a) {
				let that = this;
				that.$.navigateTo({
					url: "/community/community/detail?id=" + a.currentTarget.dataset.id,
					success: function(a) {},
					fail: function(a) {},
					complete: function(a) {}
				})
			...
			navBack() {
				// uni.navigateTo({
				// 	url: '/community/community/userspace?uid='+ this.options.uid
				// })
