<template>
  <view class="table-main" :class="stocksList.fixedTB.length == 0 ? 'loading' : ''">
    ...
    <table class="gridtable" :class="`col${stocksList.theadList.length}`" border="0">
      <thead>
        <tr class="title">
          <td class="title-th" @click="item.name == '总得分' && controlTableSort ? sortDate() : ''" v-for="(item, index) in stocksList.theadList" :class="item.type == 'fixed' ? 'fixed' : ''">
            <text class="title-key">{{ item.name }}</text>
            <image class="iconHelp" v-if="item.isIconHelp" :src="iconHelp" mode="widthFix"></image>
            <view class="" v-if="item.name == '总得分' && controlTableSort">
              <!-- 20的为不可以点的 -->
              <u-icon v-if="!sort" name="https://qnsp.zcskjy.com/zc_images/findTools/downSort.png" color="#2979ff"></u-icon>
              <u-icon v-if="sort" name="https://qnsp.zcskjy.com/zc_images/findTools/upSort.png" color="#2979ff"></u-icon>
      ...
      <tbody v-if="stocksList.fixedTB.length !== 0">
        <tr v-for="(item, index) in stocksList.fixedTB">
          <td class="body-td" v-for="(row, i) in stocksList.theadList" :class="item.type == 'fixed' ? 'fixed' : ''" @tap="itemClick(item, i)">
            <view class="val" :style="{ color: item[row.id].fontColor }"> {{ item[row.id].val }}</view>
...
export default {
  ...
  methods: {
    sortDate(){
      this.sort = !this.sort
      this.$emit('changeSortDate', this.sort)
    },
    det (item, index) {
      if (index == 0) {
