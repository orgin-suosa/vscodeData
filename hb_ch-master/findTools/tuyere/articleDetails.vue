	...
	<!-- #endif -->
		...
		<template slot="conten-center">
				<view class="statusBarHeight" :style="{ height: statusBarHeight + 'px' }"></view>
			     <sharesAbstract v-if="authStatus &&  sharesAbstractList.length !=0" :showSharesAbstract="showSharesAbstract"
                ...
                <view class="relationETF" v-if="pageBrief">核心逻辑：</view>
								<view class="pageDetail-content-Brief">{{ pageBrief }}</view>
                <view class="relationETF" v-if="relatedstocks.length !== 0">相关ETF：</view>
                <view class="relationETF">{{ relatedstocks.join('、') }}</view>
							</view>
							...
							<view class="button-list">
								<view class="button-list-child" v-if="!authStatus" @click="priceClick">
          ...
          <view class="statement">本页面转载上述内容传递更多信息之目的，不代表长乘学院观点。长乘学院力求但不保证数据的完全准确，如有错漏请以证监会指定上市公司信息披露平台为准，各类信息服务基于人工智能算法，投资者据此操作，风险自担。</view>
				</scroll-view>
		...
		methods: {
          ...
          clsType:'FKETF'
				...
				uni.request({
					...
					success: (res) => {
						const data = JSON.parse(res.data);
						console.log('风口ETF-data', data);
						const list = data.result;
            console.log('风口ETF-list',list);
						if (res.statusCode == 200) {
              that.authStatus = list.authStatus;
							that.pageTitle = list?.title;
							that.pageTime = list?.ctime;
							that.pageBrief = list?.previewcontent;
              this.relatedstocks = []
              list.stocksInfo?.forEach(item =>{
                this.relatedstocks.push(item.zhongWenJianCheng + '(' + item.obj + ')')
              })
						}
					},
					fail: (res) => {
			...
			initShareData(){
				...
				let $href = that.$.sprintf('%s/h5/findTools/tuyere/articleDetails?uid=%d', that.Config.SiteUrl, that.userInfo.user_id + '&pageID=' + that.pageID, '&showOpenApp=true');
				
				// 如允许点击超链接跳转，则应该打开一个新页面，并传入href，由新页面内嵌webview组件负责显示该链接内容
				// #ifdef APP-PLUS
				$href = that.$.sprintf('%s/h5/findTools/tuyere/articleDetails?uid=%d', that.Config.SiteUrl, that.userInfo.user_id + '&pageID=' + that.pageID, '&showOpenApp=true');
				// #endif
				// #ifdef MP-WEIXIN
				$href = '/h5/findTools/tuyere/articleDetails?uid=' + that.userInfo.user_id + '&pageID=' + that.pageID  + '&showOpenApp=true';
				// #endif
				
				var path, title, image, shareText;
				
				if (that.$.isNull(that.ProductInfo)) {
					title = that.pageTitle || '风口ETL';
