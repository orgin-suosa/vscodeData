	...
	export default {
		...
		data() {
			return {
				...
				toolsList: [
					...
					title: [
					  {
					      image: `${getApp().globalData.qnUrl}/zc_images/findTools/lhb-textTitle.png`+ this.$FT.versionKey(),
					    },
					    {
					      image: `${getApp().globalData.qnUrl}/zc_images/findTools/lhb-title.png`+ this.$FT.versionKey(),
					    },
					],
					  giftTitle: ``,
		...
		methods: {
			...
			async init(option) {
				...
				this.shareIcon = 'share-square';
				//  #endif
				// console.log('狙击龙虎榜-option', option);
			...
			panzhongbaoShop() {
				...
				uni.request({
					...
					success: (res) => {
						...
						// console.log('支付相关-list', result);
						// // // console.log('获取首页banner-page_code', page_code);
						if (res.statusCode == 200) {
							let list = result.lhb;
							console.log('支付相关-狙击龙虎榜', list);
			...
			panzhongbaoDetail() {
				...
				// console.log('狙击龙虎榜详情-入参', params);
				// console.log('狙击龙虎榜详情-url', that.Config.panzhongbaoDetail);
				uni.request({
					...
					data: params,
					success: (res) => {
						// console.log('狙击龙虎榜详情-res', res);
						const data = JSON.parse(res.data);
						// console.log('狙击龙虎榜详情-data', data);
						const list = data.result;
						// console.log('狙击龙虎榜详情-list', list);
						// // console.log('获取首页banner-page_code', page_code);
						if (res.statusCode == 200) {
					...
					complete: () => {
						...
						// console.log('狙击龙虎榜-刷新涨跌幅-入参 :that.stockCode', that.stocks, that.authStatus);
			...
			getSystemInf() {
				...
				console.info("getSystemInf========",info)
			...
			priceClick(text) {
				const that = this;
				// console.log('狙击龙虎榜-productId', text, that.productId);
				if (!this.hasLogin) {
					uni.navigateTo({
			...
			initShareData(){
				...
				let $href = that.$.sprintf('%s/h5/findTools/dragonTigerList/articleDetails?uid=%d', that.Config.SiteUrl, that.userInfo.user_id + '&pageID=' + that.pageID, '&showOpenApp=true');
				
				// 如允许点击超链接跳转，则应该打开一个新页面，并传入href，由新页面内嵌webview组件负责显示该链接内容
				// #ifdef APP-PLUS
				$href = that.$.sprintf('%s/h5/findTools/dragonTigerList/articleDetails?uid=%d', that.Config.SiteUrl, that.userInfo.user_id + '&pageID=' + that.pageID, '&showOpenApp=true');
				// #endif
				// #ifdef MP-WEIXIN
				$href = '/h5/findTools/dragonTigerList/articleDetails?uid=' + that.userInfo.user_id + '&pageID=' + that.pageID  + '&showOpenApp=true';
				// #endif
				
				var path, title, image, shareText;
				
				if (that.$.isNull(that.ProductInfo)) {
					title = that.pageTitle || '狙击龙虎榜';
