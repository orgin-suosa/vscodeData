<template>
	<!-- #ifndef H5-->
	<view class="articleDetail articleDetail_Nh5"
		:class="(authStatus || isFree) &&  popupSharesList.length !=0  && stocks!= null && stocks!= ''? 'articleDetailHidden' : ''">
		<!-- #endif -->
		<!-- #ifdef H5-->
		<view class="articleDetail articleDetail_h5"
			:class="(authStatus || isFree) &&  popupSharesList.length !=0  && stocks!= null && stocks!= '' ? 'articleDetailHidden' : ''">
			<!-- #endif -->

			<status-bar :backgroundColor="PageNav.window.navigationBarBackgroundColor"></status-bar>
			<toolTemplate ref="toolTemplate" title="详情" :navbarType="navbarType" :shareDataDefault="shareData"
				@priceClick="priceClick" :productId="productId" :totalOrderStatus="totalOrderStatus" :safeTip="safeTip"
				:totalDiscountPrice="totalDiscountPrice" :totalOriginalPrice="totalOriginalPrice"
				:buttonList="buttonList" @navBack="navBack" @rightClick="onShareBox">
				<template slot="conten-center">
					<view class="statusBarHeight" :style="{ height: statusBarHeight + 'px' }"></view>
					<sharesAbstract
						v-if="(authStatus || isFree) &&  popupSharesList.length !=0 && stocks!= '' && stocks!= null"
						:showSharesAbstract="showSharesAbstract" :sharesAbstractList="sharesAbstractList"
						@navigateClick="navigateClick"></sharesAbstract>
					<scroll-view class="articleDetail-scroll" :class="!authStatus && !isFree ? 'priceConent' : ''"
						...
						<view class="articleDetail-scroll-content" :style="{ paddingBottom: safeAreaHeight + 'px' }">
							<view class="pageDetail">
								...
								<view class="page-tip">{{ pageTip }}</view>
							</view>
							...
							<view class="popup-sharesList">
								<view class="popup-sharesList-child" @click="sharesListClick(item)"
									...
									<view class="popup-sharesList-child-left">
										<view class="popup-sharesList-child-left-name">{{ item.zhongWenJianCheng }}
										</view>
										<view class="popup-sharesList-child-left-code">{{ item.obj }}</view>
									</view>
									<view class="popup-sharesList-child-right"
										:class="item.zhangFu < 0 ? 'sharesFall' : ''">
										<view class="popup-sharesList-child-addnember">{{ item.zuiXinJia }}</view>
										<view class="popup-sharesList-child-percent" v-if="item.zhangFu > 0">
											+{{ item.zhangFu }} %
										</view>
										<view class="popup-sharesList-child-percent" v-if="item.zhangFu <= 0">
											{{ item.zhangFu }} %
				...
				<template slot="priceBottom">
					<priceBottom v-if="!authStatus && !isFree" :productId="productId" :safeTip="safeTip"
						:totalDiscountPrice="totalDiscountPrice" :totalOriginalPrice="totalOriginalPrice"
						:buttonList="buttonList" @priceClick="priceClick"></priceBottom>
				</template>
			</toolTemplate>
		</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				pageTime: '',
				pageBrief: '',
				pageTip: '本页面转载上述内容传递更多信息之目的，不代表长乘学院观点。长乘学院力求但不保证数据的完全准确，如有错漏请以证监会指定上市公司信息披露平台为准，各类信息服务基于人工智能算法，投资者据此操作，风险自担',
				authStatus: true,
				isFree: false,
				pageContent: '',
				pageJieshuo: '',
		...
		methods: {
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.statusCode == 200) {
							let jgjj = list.jgjj;
							that.totalDiscountPrice = jgjj.unitPrice;
							that.totalOriginalPrice = jgjj.marketValue;
							that.totalOrderStatus = jgjj.orderStatus;
			...
			panzhongbaoDetail() {
				...
				uni.request({
					url: that.Config.tuyereETFDetail,
					method: 'get',
					dataType: 'jsonp',
					data: params,
					success: (res) => {
						...
						if (res.statusCode == 200) {
							...
							that.pageBrief = list?.summary;
							that.pageJieshuo = list?.content;
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.statusCode == 200) {
							...
							console.log('涨跌幅-data-that.popupSharesList', that.popupSharesList, that
								.popupSharesList.length);
						}
					},
					fail: (res) => {
			...
			uparseNavigate(href, e) {
				...
				let that = this;
				uni.navigateTo({
					url: '/findTools/mechanism/articleDetails?pageID=' + pageID,
				});
				return;
				// do something
			},
			...
			initShareData() {
				...
				let $href = that.$.sprintf('%s/h5/findTools/mechanism/articleDetails?uid=%d', that.Config.SiteUrl, that
					.userInfo.user_id + '&pageID=' + that.pageID, '&showOpenApp=true');

				// 如允许点击超链接跳转，则应该打开一个新页面，并传入href，由新页面内嵌webview组件负责显示该链接内容
				// #ifdef APP-PLUS
				$href = that.$.sprintf('%s/h5/findTools/mechanism/articleDetails?uid=%d', that.Config.SiteUrl, that
					.userInfo.user_id + '&pageID=' + that.pageID, '&showOpenApp=true');
				// #endif
				// #ifdef MP-WEIXIN
				$href = '/h5/findTools/mechanism/articleDetails?uid=' + that.userInfo.user_id + '&pageID=' + that.pageID +
					'&showOpenApp=true';
				// #endif
...
<style lang="scss">
	...
	::v-deep .toolTemplate_h5 {
		.toolTemplate-content-price {
			// padding-bottom: calc(160rpx + $openAppHeight);
	...
	.articleDetail_h5 {
		.articleDetailCharge {
			::v-deep .toolTemplate-content-price {
				padding-bottom: 0;
			}
		}
