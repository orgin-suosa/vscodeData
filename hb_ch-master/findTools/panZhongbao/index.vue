...
export default {
  ...
  data() {
    return {
      ...
      iconTitle: `${getApp().globalData.qnUrl}/zc_images/findTools/page-title.png`,
  ...
  methods: {
     ...
     //  console.log('盘中宝-panzhongbaoList-url', that.Config.panzhongbaoList);
      uni.request({
        ...
        success: (res) => {
         ...
         //  console.log('盘中宝-panzhongbaoList-data', data);
          ...
          if (res.statusCode == 200) {
            ...
            if (type == 0) {
             ...
             //  console.log('盘中宝-panoSelectedList :that.firstShares', that.firstShares);
              ...
              //   });
              // });
             //  console.log('盘中宝-panoSelectedList :that.selectedShareList', that.selectedShareList);
            }
    ...
    navigateDetails(id) {
     //  console.log('详情-参数-navigateDetails', id);
      ...
      // });
      uni.navigateTo({
        url: '/findTools/panZhongbao/articleDetails?pageID=' + id,
      });
    },
    navigate(href) {
	...
	onShareBox() {
		...
		let $href = that.$.sprintf('%s/h5/findTools/panZhongbao/index?uid=%d', that.Config.SiteUrl, that.userInfo.user_id,'&showOpenApp=true');
		
		// 如允许点击超链接跳转，则应该打开一个新页面，并传入href，由新页面内嵌webview组件负责显示该链接内容
		// #ifdef APP-PLUS
		$href = that.$.sprintf('%s/h5/findTools/panZhongbao/index?uid=%d', that.Config.SiteUrl, that.userInfo.user_id, '&showOpenApp=true');
		// #endif
		// #ifdef MP-WEIXIN
		$href = '/h5/findTools/panZhongbao/index?uid=' + that.userInfo.user_id + '&showOpenApp=true';
		// #endif
		that.setData({
			shareData: {
				shareTitle: '盘中宝',
...
$navbar-bg2: 'https://qnsp.zcskjy.com/zc_images/findTools/navbar-bg2.png';
.panZhongbao-content{
	::v-deep  .toolTemplatenav {
		&-navbar {
		  width: 750rpx;
		  height: 100%;
		  // height: calc(165rpx + var(--status-bar-height));
		  background: url($navbar-bg2) no-repeat center;
		  background-size: 498rpx 45rpx;
