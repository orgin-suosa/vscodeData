	...
	<view class="page">
		<block v-if="(isdata)">
			...
			<scroll-view class='u-pa1 fix-titlenview-trans' scroll-y="true" :scroll-top="scposition" @scroll="scrolltoupper">
				...
				<!--评论-->
				<view class="evaluate hide" v-if="ProductInfo.last_comment_rows.length > 0 ">
					<view class="evaluate-top">
						...
						<navigator :url="'/pages/product/commentlist?id=' + (proId)" class="evaluate-right">
							<label class="right">{{__('全部评价')}}</label>
								<image style="width:16upx;height:28upx;position: absolute;margin-left:20upx;margin-top:10upx;" src="https://static.shopsuite.cn/xcxfile/appicon/goto.png" />
						</navigator>
					</view>
				...
				<block v-if="true">
				<!--问答-->
				<view :class="'m-panel m-panel-access m-comment'" v-if="false" :data-product_id="proId" @click="listFaq">
				...
				<view :class="'m-panel m-panel-access m-comment'" v-if="false">
			...
			<view class="m-footer-btn bottom-safearea" :class="ProductInfo.is_virtual ? 'm-fg-btn' : ''" v-if="(ProductInfo.activityType!='GROUPBOOKING')">
				...
				<block v-if="(ProductInfo.is_virtual)">
					<view class="m-footer-btn-list hide" >
				...
				<block v-else>
					<view class="m-footer-btn-list">
						<view class="m-footer-btn-item hide" :data-id="(proId)" :data-name="(store_info.store_name)" :data-keyword="(ProductInfo.keyWord)"
						...
						<view data-url="/pages/cart/cart" class="m-footer-btn-item hide"  @click="goTabBar">
							<view class="m-footer-btn-icon">
					...
					<block v-else>
						<view class="m-footer-btn-main hide" @click="ckselectsp">
							{{__('加入购物车')}}
						</view>
						<!-- <view v-else class="m-footer-btn-main" @click.stop="addCart">
			      {{__('加入购物车')}}
