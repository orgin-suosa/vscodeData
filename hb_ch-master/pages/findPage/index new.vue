<template>
	<view class="container">
		...
		<scroll-view scroll-y="true" :style="{ height: clientHeight + 'px' }" @scrolltolower="scrolltolower">
			...
			<findTeacher ref="findteach" v-if="currentIndex==0" :viewHeight="clientHeight" :followPartHeight="followPartHeight"></findTeacher>

            <lookHotTeacher ref="lookhot" v-if="currentIndex==1" :viewHeight="clientHeight" ></lookHotTeacher>
...
<script>
	...
	export default {
		...
		onShow() {
			console.info('---findgage---hasLogin----',this.hasLogin);
			console.info('---findgage---firstLoad----',this.firstLoad);
			if (this.firstLoad == false) {
				// console.info('---findgage---loadRefresh----');
			    if (this.currentIndex == 0){
					console.info('---findgage--0---loadRefresh----');
			    	this.$refs.findteach.loadRefresh();
			    }else if (this.currentIndex == 1){
					console.info('---findgage--1---loadRefresh----');
			    	this.$refs.lookhot.loadRefresh();
			    }else if (this.currentIndex == 2){
					console.info('---findgage--2---loadRefresh----');
			    	this.$refs.followT.loadRefresh();
			    }
			}else{
