		...
		<view class="m-cells m-cells-form">
			<view class="m-cell m-cell-select-before">
				...
				<view class="m-cell-bd">
					 <picker @change="bindPickerChange" :value="index" :range="invoice_category_row" range-key="name">
						<view class="uni-input">{{invoice_category_row[index].name}}</view>
					 </picker>
				</view>
			</view>
			<view class="m-cell">
				...
				<view class="m-cell-bd">
					<radio-group class="uni-flex" name="invoice_type"  @change="onInvoiceType">
							<radio value="0" :checked="invoice_type==0"/>{{__('个人')}}
							<radio value="1" :checked="invoice_type==1"  />{{__('企业')}}
					</radio-group>
				</view>
			</view>
			<view class="m-cell">
				<view class="m-cell-hd"><label for="" class="u-label">{{0==invoice_type ? __('个人名称') : __('单位名称')}}</label></view>
				<view class="m-cell-bd">
	...
	export default {
		...
		data() {
			return {
				...
				seladstr: "",
				options:{},
				invoice_category_row: [], //{name:'增值税专用发票', id:2}
				index:0
			}
		},
		...
		onLoad: function(options) {
			...
			this.invoice_category_row = [{name: this.__('普通发票'), id:1}, {name:'增值税专用发票', id:2}];  
					...
					success: function(data, status, msg, code) {
						if (200 == status) {
							that.setData({
								...
								index: data.invoice_category == 1 ? 0 : 1,
							});
						}
					}
				});
		...
		methods: {
			...
			submitinfo: function() {
				...
				*/
				...
				var params = {
					...
					invoice_kind                : that.invoice_kind            ,
					invoice_category            : that.invoice_category        ,
									...
									success: function() {
										if (options.issub) {
											that.$.navigateBack(1, function() {
												...
												data['invoice_header'] = that.sprintf('%s - %s', data.invoice_category==1 ? that.__('普通发票') : that.__('增值税专用发票'), data.invoice_title);
			...
			onInvoiceType:function(e){
				this.invoice_type = e.target.value
				var that = this;
			},
			bindPickerChange:function(e){
				this.index = e.target.value
				var that = this;
				this.invoice_category = this.invoice_category_row[this.index].id;
				/*
