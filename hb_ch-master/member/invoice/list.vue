<template>
	<view class="page">
		<view v-if="(isdata)" class="m-ad-list">			
			...
			<button class="u-btn u-btn-default" @click="addInvoice" style="margin-top:50upx;">{{__('添加发票')}}</button>
			<button class="u-btn u-btn-secondary" @click="selectInvoice" style="margin-top:50upx;" v-if="options.issub">{{__('不开发票')}}</button>
		</view>
		...
		methods: {
			...
			selectInvoice: function (obj) {
					...
					this.setData({
						...
						invoice_header: that.sprintf('%s - %s', obj.currentTarget.dataset.invoice_category==1 ? this.__('普通发票') : this.__('增值税专用发票'), obj.currentTarget.dataset.invoice_title, obj.currentTarget.dataset.invoice_title)
					});
