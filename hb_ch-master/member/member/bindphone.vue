<template>
	<view class="register">
		...
		<view class="top_title">
		 ...
		 <text class="tip_text">根据账号相关管理规定要求，请您绑定手机号</text>
			<!-- 主体 -->
			<view class="main">
				<wInput
					v-model="user_mobile"
			...
			<wButton 
				:text="__('绑 定')"
				:bgColor="__('#FC4B21')"
				@click.native="submitdata()" style="margin-top: 30rpx;"
			></wButton>
    ...
    export default {
        ...
        methods: {
			...
			submitdata: function() {
					...
					else if (that.$.tel(this.user_mobile, this.user_intl))
						if (false && this.user_mobile == this.userInfo.user_mobile) {
					...
					// 	});
						...
						var params = {
							mobile: that.user_intl + this.user_mobile,
							//password: this.password,
							auth_code: this.mobile_code,
							user_intl: that.user_intl
						};
						
						console.log('submitdata=1',params)
			
						that.$.request({
							url: this.Config.URL.account.bind_mobile,
							data: params,
							success: function(data, status, msg, code) {
								console.log('submitdata=2',data)
								if (status == 200) {
									let user_info = {};
									user_info['user_mobile'] = that.user_mobile;
									user_info['as'] = 1;
									if(data.commonResult!=null && data.commonResult!='null' && typeof(data.commonResult) != undefined){
										var id = that.$.setStorageSync('uid', data.commonResult.user_id);
										var key = that.$.setStorageSync('ukey', data.commonResult.key);
										// #ifdef H5
										$cookies.set('uid', data.commonResult.user_id);
										$cookies.set('ukey', data.commonResult.key);
										// #endif
										that.login(data.commonResult);
									}else{
