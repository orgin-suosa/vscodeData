	...
	export default {
		...
		methods: {
            ...
            inputICard: function (e) {
                ...
                {
                    this.setData({isCardNo: !0, user_idcard: e.detail.value});
                    return
                }
                this.IdentityCodeValid(e.detail.value) ? this.setData({isCardNo: !0}) : this.setData({isCardNo: !1}), this.setData({user_idcard: e.detail.value})
            },
        ...
        if (!/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/.test(this.user_email))
        {
            ...
            inputphone: function (e) {
				let that = this;
                this.setData({user_mobile: e.detail.value}), that.$.isNull(e.detail.value) ? this.setData({isUserMobile: !1}) : /^1[23456789]\d{9}$/.test(e.detail.value) ? this.setData({isUserMobile: !0}) : this.setData({isUserMobile: !1})
            },
                ...
                else if (this.isSend)
                {
                    ...
                    that.$.request({
                        ...
                        success: function (data, status, msg, code) {
                            ...
                            {
                                that.setData({isSend: !0}), that.$.alert(msg)
                            }
                        }
                    });
                }
