            ...
            <view class="m-media-box m-media-box-small-appmsg">
                <view class="m-cells">
                    <block v-for="(item, idx) in message_rows" :key="idx">
                        <view :class="['m-cell','m-cell-access', item.message_is_read?'':'active']" @click="tochat(item.user_other_id)">
                            ...
                            <view class="m-cell-bd m-cell-primary">
                                <p class="m-cell-primary-content" style="">
									...
									<label class="user_nickname">{{item.user_other_nickname}}</label>
								...
								<view v-if="item.user_other_id != '10001'">
    ...
    export default {
        ...
        methods: {
			...
			roomList() {
				let that = this;
				that.$.request({
					url: that.Config.URL.sns.roomList,
					success: function(data, status, msg, code) {
						console.log("roomList", data)
						// that.setData({
						// 	sysMsg:data.sysMsg,
						// 	userMsg:data.userMsg,
						//     message_rows: that.message_rows.concat(data.userMsg),
						// });
						// that.message_rows = [...data.sysMsg[0],...data.userMsg];
						    that.message_rows = data.userMsg;
							that.sysMsg = data.sysMsg[0];
							that.message_rows = that.message_rows.concat(that.sysMsg);
							for (var r = 0; r < that.message_rows.length; r++) {
							   	that.message_rows[r]['message_time_str'] = dateUtil.dateUtils.format(that.$.datetimeFormatter(that.message_rows[r]['message_time']));
							}
							console.log("roomList-message_rows", that.message_rows )
					},
					fail: () => {},
					complete: () => {
					},
