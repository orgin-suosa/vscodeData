<template>
    <view class="page">
        ...
        <scroll-view class="m-product-all" scroll-y="true" @scrolltolower="scrollbottom" style="width:100%;height:100%;position: absolute;">
            <view class="header">
                <view class="num">{{resource.user_points}}<label style="font-size:34upx">{{__('分')}}</label></view>
            ...
            <view v-if="(list.length>0)"  class="m-orderlist" >
                <view class="detail" v-for="(items, index) in list"  :key="index">
                    <view class="detail-left">
                        ...
                        <view class="time" style="margin-top:20upx">{{items.points_log_time}}</view>
                    </view>

                    <view class="detail-right">
                        <label class="money add pprice" v-if="(items.points_kind_id == 1)">+{{Math.abs(items.points_log_points)}}</label>
