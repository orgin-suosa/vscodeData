...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState(['Config', 'hasLogin', 'userInfo', 'plantformInfo', 'getMsg','newMsgNum'])
        },
        watch: {
			userOtherRead: {
			      handler(newName, oldName) {
			        this.userOtherRead = newName;
			        this.$forceUpdate();
			      },
			      deep: true,
			      immediate: true,
            ...
            getMsg: function (val) {
                ...
                if (that.chat_to_puid == val.id)
                {
                    ...
                    //设为已读
					let msgReadList = that.msgList.forEach(function (item, i, array) {
						console.log("获取消息-设为已读-isRead",i,array, !item.isRead)
						array[i] = {
							...item,
							isRead:true
						}
						console.log("获取消息-设为已读-isRead--array",array)
						return array;
					});
					console.log("获取消息-设为已读-apirequest-msgList",msgReadList);
                }
        ...
        methods: {
            ...
            screenMsg(msg)
            {
                ...
                {
                    // 系统消息
					console.info('-------------接受消息-系统消息',msg.msg.type)
                    switch (msg.msg.type)
                    {
                        case 'text':
            ...
            ,
			systemReceipt(msg){
				console.log("处理用户发送的系统回执",msg)
				// let that = this;
				// 	this.msgList.forEach(function (item, i, array) {
				// 	console.log("处理用户发送的系统回执-item.isRead",i,array, !item.isRead)
				// 	if(!item.isRead){
				// 		that.msgList[i].push({
				// 			isRead:true
				// 		})
				...
				console.log("处理用户发送的系统回执--this.msgList",this.msgList)
			},
            // 添加系统文字消息到列表
