        ...
        <view class="row" v-for="(row, index) in msgList" :key="index" :id="'msg' + row.msg.id">
          ...
          <block v-if="row.type == 'user'">
            ...
            <view class="my" v-if="row.msg.userinfo.uid == myuid">
              <!-- 左-消息 -->
              <view class="left">
                <view class="">{{ row.isRead ? '已读' : ' 未读' }}</view>
                <!-- 文字消息 -->
                <view v-if="row.msg.type == 'text' || row.msg.type == 'system' " class="bubble">
                    ...
                    <view class="m-product-info1" style="display: flex; flex-flow: column; justify-content: space-between">
                      ...
                      <view class="m-product-price1">
                        <view class="money">
                          <block>
                            <label>{{ __('￥') }}</label
                            ><label style="">{{ number_format(row.msg.content.item_unit_price, 2) }}</label>
                          </block>
                        </view>
                      </view>
                    </view>
      ...
      <view class="more-layer" :class="{ hidden: hideMore }">
        <view class="list">
          ...
          <view class="box" @tap="yuyintonghua">
            <image style="font-size: 16px; width: 32px; height: 32px" src="https://static.shopsuite.cn/xcxfile/appicon/im/img/more/yuyintonghua.png"></image>
          </view>
          <!-- #endif -->

          <view class="box" @tap="weizhi" v-if="false">
            <image style="font-size: 16px; width: 32px; height: 32px" src="https://static.shopsuite.cn/xcxfile/appicon/im/img/more/weizhi.png"></image>
          </view>
          ...
          <view class="box" @tap="yuyinshuru" v-if="false">
            <image style="font-size: 16px; width: 32px; height: 32px" src="https://static.shopsuite.cn/xcxfile/appicon/im/img/more/yuyinshuru.png"></image>
          </view>

          <view class="box" @tap="meShouchang" v-if="false">
            <image style="font-size: 16px; width: 32px; height: 32px" src="https://static.shopsuite.cn/xcxfile/appicon/im/img/more/me-shouchang.png"></image>
          </view>

          <view class="box" @tap="userinfo" v-if="false">
            <image style="font-size: 16px; width: 32px; height: 32px" src="https://static.shopsuite.cn/xcxfile/appicon/im/img/more/userinfo.png"></image>
          </view>
    ...
    <view class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
      ...
      <view class="textbox">
        ...
        <view class="text-mode" :class="isVoice ? 'hidden' : ''">
          <view class="box">
            <textarea auto-height="true" placeholder="我也来说两句…" v-model="textMsg" @focus="textareaFocus" />
          </view>
...
export default {
  ...
  computed: {
	...
	...mapMutations(['updateIsNewMsg']),
	  scrollHeight: function () {
	      let sys = uni.getSystemInfoSync();
	      let winWidth = sys.windowWidth;
	      let winrate = 750 / winWidth;
	      let winHeight = parseInt(sys.windowHeight * winrate);
	      return winHeight -  this.$FT.pxToRpx(70) - 44;
	    },
  },
  watch: {
    getMsg: function (val) {
      ...
      if (that.chat_to_puid == val.id) {
        apirequest.setMsgRead(lastid, (e) => {
          console.log('获取消息-设为已读-setMsgRead', e);
		  that.$store.commit('updateIsNewMsg',false);
        });
        //设为已读
        that.msgList.forEach(function (item, i, array) {
          // console.log('获取消息-设为已读-isRead', i, array, !item.isRead);
          // array[i] = {
          //   ...item,
          //   isRead: true,
          // };
        ...
        console.log('获取消息-设为已读---that.msgList', that.msgList);
      }
  ...
  onLoad(options) {
    ...
    this.$.request({
      ...
      success: (data, status, msg, code) => {
        console.info('msg_config-初始化信息配置接口', data,data.user_other_info, data.user_other_info.user_nickname);
        if (status == 200) {
          if (!options.name) {
          ...
          // this.chattitle = options.title;
		          console.info('msg_config-初始化信息配置接口-chattitle',this.chattitle)
		  this.getMsgList();
          // 将未读条数归零
          this.autoSendSystemReceipt();
  ...
  methods: {
    ...
    screenMsg(msg) {
      console.info('-------------接受消息-screenMsg', msg);
      //从长连接处转发给这个方法，进行筛选处理

      if (msg.type == 'system') {
        // 系统消息yidu
        console.info('-------------接受消息-系统消息',msg, msg.msg.type);
        switch (msg.msg.type) {
          case 'text':
          ...
          case 'redEnvelope':
            ...
            break;
		 case 'system':
			this.addSystemReceipt(msg);
			  break;
        }
      } else if (msg.type == 'user' || msg.type == 'friend') {
        console.info('-------------接受消息-user/friend', msg.msg.type);
        // 用户消息
        switch (msg.msg.type) {
          case 'text':
            this.addTextMsg(msg);
    ...
    getMsgList(callback) {
      ...
      this.$apiconfig.getrecentmsg_f(params, (data, status, msg, code) => {
        console.info('getrecentmsg_f', data);
        console.info(status);
        console.info('getrecentmsg_f---加载初始页面消息-msg', msg);
		data.items = data.items.filter(function(item){
			console.info('getrecentmsg_f-----消息类型-', item.msg.type);
			return item.msg.type != 'system';
		})
        if (data.items.length > 0) {
          let array = [];
          list = data.items.forEach((item) => {
            // console.log("加载初始页面消息-设为已读-setMsgRead",that.chat_to_puid,item.msg.id);
            // if (that.chat_to_puid == item.msg.id)
            // {
            // 	apirequest.setMsgRead(lastid, (e)=>{
            // 	console.log("加载初始页面消息-设为已读-setMsgRead",e);
            // 	})
            // 	//设为已读
            // 	console.log("加载初始页面消息ss-设为已读-apirequest",apirequest);
            // }
            let items = {
              ...item,
              isRead: true,
            };
            // console.log("加载初始页面消息-设为已读-isRead",items);
            array.push(items);
          });

          //新消息数归零
          // that.$store.commit('resetWeidu', { type: this.chattype, val: this.chat_to_puid });
        ...
        console.info(data);
        console.info('加载初始页面消息-msg',this.msgList);
        ...
        this.$store.commit('updateChaterAttr', {
          ...
          this.$.request({
            ...
            success: function (tmp_data, tmp_status, tmp_msg, tmp_code) {
              if (tmp_status == 200) {
                // console.log("设置用户消息全部为已读");
              }
            },
          });
    },
	autoSendSystemReceipt(){
		let that = this;
		let params = {
			recently_flag: 1,
		};
		// message_kind: 1是本人发送
		if(that.msgList.length == 0) return;
		let l=that.msgList.length - 1;
		let i = l;
		// 统计一下未读的消息
		if(this.msgList[i].msg.type == 'system'){
			i--;
		}
		console.log("发送已读回执",this.msgList.length,this.msgList,this.msgList[i].msg.type == 'system',this.msgList[i].message_kind )
		that.$.request({
			...
			success: function(data, status, msg, code) {
				if (!data.msg_row || (data.msg_row && 1 == data.msg_row.message_type)) {
					...
					if(this.msgList[i].message_kind == 2){
						console.log("发送已读回执---------------")
						this.sendSystemReceipt('已读回执', 'system');
						// that.msgSetRead();
					}
				}
    ...
    // 发送系统回执
    sendSystemReceipt(content) {
      console.log('发送系统回执', content);
      let msg = { text: content };
      this.sendMsg(msg, 'system');
	  
	  // let row = { type: 'system', msg: { type: 'system', content: content } };
	  // this.screenMsg(row);
    },
    // 发送消息
    ...
    addSystemReceipt(msg) {
		let that = this;
	      console.log('处理用户发送的系统回执', msg,that.msgList.length);
		that.msgList.forEach(function (item, i, array) {
			that.msgList[i] = {
			  ...item,
			  isRead: true,
      ...
      console.log('处理用户发送的系统回执--this.msgList', this.msgList,that.msgList.length);
    },
    // 添加系统文字消息到列表
...
@import '@/styles/community/circleChat.scss';
