      ...
      >
        ...
        <view class="row" v-for="(row, index) in msgList" :key="index" :id="'msg' + row.msg.id">
         ...
         <!--       <view class="">{{ row.isRead ? '已读' : ' 未读' }}</view> -->
                ...
                <!-- 图片消息 -->
                <view v-if="row.msg.type == 'img'" class="bubble img" @tap="showPic(row,index)">
					
     <!--             <image :src="row.msg.content.url" :style="{ width: row.msg.content.w + 'px', height: row.msg.content.h + 'px' }"></image> -->
<!-- 	                <image :src="row.message_content" :style="{ width: row.msg.content.w + 'px', height: row.msg.content.h + 'px' }"></image> -->
					<image v-if="row.msg.content.url != undefined && row.msg.content.url != null " :src="row.msg.content.url" :style="{ width: row.msg.content.w + 'px', height: row.msg.content.h + 'px' }"></image>
					<image v-else :src="row.message_content" :style="{ width: row.msg.content.w + 'px', height: row.msg.content.h + 'px' }"></image>
            ...
            <view class="other" v-if="row.msg.userinfo.uid != myuid">
              <!-- 左-头像 -->
              <view class="left">
                <image v-if="row.msg.userinfo.face != '' " :src="row.msg.userinfo.face"></image>
				<image v-else :src="otherAvatar"></image>
    ...
    <!-- 底部输入栏 -->
    <view class="input-box" v-if="options.uid  != '10001' " :class="popupLayerClass" @touchmove.stop.prevent="discard">
      <!-- H5下不能录音，输入栏布局改动一下 -->
      <!-- #ifndef H5 -->
      <!--            <view class="voice">
    ...
    export default {
        ...
        data()
        {
            return {
                ...
                ispage: !1,
                flag: !1,
				otherAvatar: `${getApp().globalData.qnUrl}/zc_images/WXshare.jpg` + this.$FT.versionKey(),
            };
        },
        computed: {
        ...
        methods: {
			...
			setMyMsgNum(num) {
				...
				that.$.request({
					...
					success: function(data, status, msg, code) {
						console.log("setMyMsgNum-上条未读消息数目", data)
						if (status == 200) {
            ...
            showPic(row,index)
            {
				...
				if(msg.content.url != undefined && msg.content.url != null){
					url = msg.content.url;
				}else if(this.msgImgList[i] != undefined && this.msgImgList[i] != null){
					url = this.msgImgList[i];
				}else{
					url = row.message_content
				}
				console.log("预览图片-row,url",row,url)
				console.log("预览图片-this.msgImgList[i],i, msg.content.url",this.msgImgList[i],i, msg.content.url)
				uni.previewImage({
				    indicator: "none",
				    current:  url,
			...
			handleError(e,i){
				this.msgList[i].row.msg.content.url=this.msgList[i].row.message_content;
			},
			navigate(href, e) {
			  // do something
			  console.info(href);
