  ...
  <view>
    <view class="content" @touchstart="hideDrawer">
      ...
      >
        ...
        <view class="row" v-for="(row, index) in msgList" :key="index" :id="'msg' + row.msg.id" v-if="row.msg.type != 'system'">
          <!-- 系统消息 -->
  ...
  onLoad(options) {
    ...
    this.$.request({
      ...
      success: (data, status, msg, code) => {
        console.info('msg_config-初始化信息配置接口', data);
        if (status == 200) {
          if (!options.name) {
