...
export default {
  ...
  created() {
    ...
    const columnsLength = 60;
    const columns = [];
    for (let i = 40; i <= columnsLength;) {
      i = i + 5;
      const obj = {
        text: i + '岁',
        age: i,
  ...
  methods: {
    ...
    confirmAge(row) {
      ...
      console.log("提交年龄-extendField:", this.extendField);
      console.log("提交退休年龄:", this.extendField, row.age);
      this.ageValue = row.age;
      this.$emit('changeValue', this.extendField, row.age);
      // this.$store.commit('mutationsReduceRetireAge', row.age);
      this.showAge = false;
    },
    // 开启年龄弹窗
    openAge() {
